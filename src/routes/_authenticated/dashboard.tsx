...
import { getTasks, getProjects, getTimesheets } from "@/server-fns/functions";
            ...
            <ul className="divide-y divide-border">
              {myTasks.slice(0, 6).map((t) => {
                ...
                return (
                  <li
                    key={t.id}
                    className="grid grid-cols-[auto_1fr_auto_auto] items-center gap-3 px-4 py-2 text-xs hover:bg-surface-2"
                  >
                    ...
                    <span className={`rounded border px-1.5 py-0.5 text-[10px] ${pm.chip}`}>
                      {pm.label}
                    </span>
              ...
              {workload.map((w) => {
                ...
                return (
                  <li key={w.id} className="flex items-center gap-3 px-4 py-2 text-xs">
                    <span
                      className={`h-1.5 w-1.5 rounded-full ${overloaded ? "bg-destructive" : "bg-success"}`}
                    />
